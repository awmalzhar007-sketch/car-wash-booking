...
export default function StaffDashboardPage() {
  ...
  // Selected date for viewing any day ("" means today)
  const [selectedDate, setSelectedDate] = useState<string>("");

  const loadSchedule = async (isManualRefresh = false, targetDate?: string) => {
    try {
      const dateToUse = targetDate !== undefined ? targetDate : selectedDate;
      const hasCachedData = !dateToUse && typeof window !== "undefined" && !!sessionStorage.getItem("foam_staff_data");
      if (isManualRefresh || hasCachedData) setRefreshing(true);
      else setLoading(true);
      setError(null);

      const url = dateToUse ? `/api/staff/schedule?date=${encodeURIComponent(dateToUse)}` : "/api/staff/schedule";
      ...
      setData(json);
      if (!dateToUse && typeof window !== "undefined") {
        try {
          sessionStorage.setItem("foam_staff_data", JSON.stringify(json));
        } catch (e) {}
      }
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 space-y-6">
        {/* Date Selector & Operations Day Controller */}
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs text-slate-500 font-medium">
                  {language === "ar" ? "تاريخ العمليات المعروض:" : "Viewing Operations For:"}
                </span>
                ...
                {(!selectedDate || selectedDate === data?.today) ? (
                  ...
                  <span className="text-[10px] font-bold bg-amber-100 text-amber-800 px-2.5 py-0.5 rounded-full flex items-center gap-1">
                    ...
                    {language === "ar" ? "يوم محدد من السجل" : "Selected History Date"}
                  </span>
                )}
              </div>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {language === "ar"
                  ? "يمكنك الانتقال لأي يوم في الشهر لعرض الحجوزات والمحطات والدخل المحقق"
                  : "Navigate to any day in the month to inspect bookings, bays timeline, and realized income"}
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100/80 border border-slate-200 rounded-xl px-3 py-2 text-xs transition-colors">
              <Calendar className="w-3.5 h-3.5 text-blue-600" />
              <input
                type="date"
                title={t("staff_pick_date")}
                value={selectedDate || data?.date || ""}
                onChange={(e) => handleSelectDate(e.target.value)}
                className="bg-transparent text-slate-800 font-bold focus:outline-none text-xs cursor-pointer font-mono"
              />
            </div>
            ...
            {selectedDate && selectedDate !== data?.today && (
              <button
                ...
                className="text-xs bg-slate-900 hover:bg-slate-800 text-white font-bold px-3.5 py-2 rounded-xl transition-all shadow-sm flex items-center gap-1.5"
              >
                <RotateCcw className="w-3 h-3" />
                <span>{t("staff_back_to_today")}</span>
              </button>
            )}
          </div>
        </div>
        ...
        {viewTab === "DAILY_SUMMARY" && (
          <div className="space-y-6 animate-fade-in">
            ...
            {/* Best Day of the Month Highlight Card */}
            {data?.financials?.bestDay && (
              <div className="bg-gradient-to-br from-amber-50 via-yellow-50/60 to-white border border-amber-300 rounded-2xl p-5 shadow-sm space-y-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-base font-black text-slate-900">
                          {t("staff_best_day_title")}
                        </h3>
                        <span className="text-[10px] font-black bg-amber-200 text-amber-900 px-2.5 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1">
                          <span>🏆</span>
                          <span>{t("staff_best_day_badge")}</span>
                        </span>
                      </div>
                      <p className="text-xs text-slate-600 mt-0.5 font-bold font-mono">
                        {data.financials.bestDay.date}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="text-start sm:text-end">
                      <p className="text-[11px] text-slate-500 font-semibold">
                        {language === "ar" ? "أعلى دخل يومي محقق:" : "Peak Daily Revenue:"}
                      </p>
                      <p className="text-2xl sm:text-3xl font-black text-amber-700">
                        {formatPrice(data.financials.bestDay.revenue)}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleSelectDate(data.financials.bestDay.date)}
                      className="text-xs bg-amber-500 hover:bg-amber-600 text-white font-bold px-4 py-2.5 rounded-xl shadow-sm transition-all active:scale-95 flex items-center gap-1.5"
                    >
                      <span>{t("staff_view_day")}</span>
                      <ChevronRight className="w-3.5 h-3.5 rtl:rotate-180" />
                    </button>
                  </div>
                </div>
                <div className="pt-2 border-t border-amber-200/60 flex items-center justify-between text-xs text-slate-600 flex-wrap gap-2">
                  <span>
                    {language === "ar"
                      ? `تم غسيل وإنجاز ${data.financials.bestDay.count} سيارة في هذا اليوم القياسي`
                      : `${data.financials.bestDay.count} completed washes recorded on this peak day`}
                  </span>
                  <span className="text-[11px] text-amber-800 font-bold bg-amber-100/80 px-2 py-0.5 rounded">
                    {language === "ar" ? "الأعلى أداءً وإيراداً في هذا الشهر" : "Highest Performing Day This Month"}
                  </span>
                </div>
              </div>
            )}

            {/* Monthly Days Breakdown History */}
            {data?.financials?.monthlyDays && data.financials.monthlyDays.length > 0 && (
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3 flex-wrap gap-2">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                      <CalendarDays className="w-4 h-4 text-blue-600" />
                      {t("staff_monthly_history")}
                    </h3>
                    <p className="text-xs text-slate-500">
                      {language === "ar"
                        ? "سجل إيرادات وعدد سيارات كل يوم في الشهر الحالي. اضغط على أي يوم لعرض تفاصيله الكاملة."
                        : "Revenue and completed count for each day this month. Click any day to view full schedule."}
                    </p>
                  </div>
                  <span className="text-xs font-semibold bg-blue-50 text-blue-700 px-2.5 py-1 rounded-lg">
                    {data.financials.monthlyDays.length} {language === "ar" ? "أيام مسجلة" : "recorded days"}
                  </span>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  {data.financials.monthlyDays.map((day: any) => {
                    const isSelected = day.date === (data.date || data.today);
                    return (
                      <div
                        key={day.date}
                        onClick={() => handleSelectDate(day.date)}
                        className={`p-3.5 rounded-xl border transition-all cursor-pointer space-y-2 hover:shadow-md ${
                          day.isBestDay
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          {day.isBestDay && (
                            <span className="text-[10px] font-bold bg-amber-200 text-amber-900 px-1.5 py-0.5 rounded flex items-center gap-1">
                              ...
                              {language === "ar" ? "أفضل يوم" : "Peak Day"}
                            </span>
                          )}
                          {day.date === data.today && !day.isBestDay && (
                        ...
                        <div className="flex justify-between items-end pt-1">
                          <div>
                            ...
                            <p className="text-xs font-bold text-slate-700">
                              {day.count} {language === "ar" ? "سيارة" : "cars"}
                            </p>
                          </div>
                          <div className="text-end">
                            <p className="text-[10px] text-slate-400 font-semibold">
                              {language === "ar" ? "الدخل المحصل:" : "Revenue:"}
                            </p>
